...
struct DeployPerformer {
    let perform: (_ rimeDir: URL, _ fileManager: FileManager, _ strings: AppStrings) async -> DeployFeedback
...
class ConfigManager: ObservableObject {
    ...
    init(
        ...
        deployPerformer: DeployPerformer = .live
    ) {
        self.fileManager = fileManager
        self.userDefaults = userDefaults
        self.deployPerformer = deployPerformer
    ...
    private func saveSquirrelCustom() throws {
        ...
        var lines = [
            ...
            "  style/font_face: \(quoted(style.fontFace))",
            "  style/font_point: \(style.fontPoint)",
            "  style/label_font_face: \(quoted(style.labelFontFace))",
            ...
            "  style/inline_preedit: \(boolString(style.inlinePreedit))",
            "  style/inline_candidate: \(boolString(style.inlineCandidate))",
            "  style/translucency: \(boolString(style.translucency))",
            ...
            "  style/show_paging: \(boolString(style.showPaging))"
        ]

        for option in appOptions.sorted(by: { $0.bundleID.localizedStandardCompare($1.bundleID) == .orderedAscending }) {
            lines.append("  app_options/\(option.bundleID)/ascii_mode: \(boolString(option.asciiMode))")
            lines.append("  app_options/\(option.bundleID)/inline: \(boolString(option.inlineMode))")
            lines.append("  app_options/\(option.bundleID)/no_inline: \(boolString(option.noInline))")
            lines.append("  app_options/\(option.bundleID)/vim_mode: \(boolString(option.vimMode))")
        }
    ...
    private func saveDefaultCustom() throws {
        ...
        if enabledSchemas.isEmpty {
            ...
            let schemaBlock = enabledSchemas.map { item in
                """
                    - schema: \(item.id)
                      name: "\(item.name)"
                """
